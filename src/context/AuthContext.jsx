...
function AuthProvider({children}){
    ...
    async function signUp(email, password, confirm_password) {
      ...
      createUserWithEmailAndPassword(auth, email, password).then(async(result) => {
        ...
        if (token) {
					...
					localStorage.setItem('vDelivery', '');
					localStorare.setItem('vID', 0);
    ...
    return (
      <AuthContext.Provider value={{ loading, message, result, authenticated, signIn, signUp, changePassword, signOut }}>
        {children}
